...
def define_D(input_nc, ndf, which_model_netD='n_layers',
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        ...
        self.sequence_add = [nn.Sequential(*[
        ...
        self.sequence_add = nn.ModuleList(self.sequence_add)
        ...
        #     # nn.LeakyReLU(0.2, True)
        # ]
        # self.end += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]
    ...
    def forward(self, input):
        ...
        out = self.sequence[0](input[0])
        out = self.sequence_add[0](out)
        for i in range(1, self.n_layers):
            new = self.sequence[i](input[i])
            # print(i, new.shape, out.shape)
            out = out + new
            out = self.sequence_add[i](out)
        out = self.end(out)
        # print("end", out.shape)
        #print(res.shape)
        return out
